...
def extract_features(tree, entities, e1, e2) :
   ...
   if tkE1 is not None and tkE2 is not None:
      ...
      # Add features for parent
      parent1 = tree.get_parent(tkE1)
      feats.add("parent1="+ tree.get_lemma(parent1).lower())
      parent2 = tree.get_parent(tkE2)
      feats.add("parent2=" + tree.get_lemma(parent2).lower())
      if parent1 == parent2:
          feats.add("parents=same")

      # Add features for token before E1
      if tkE1>0:
          tk = tkE1-1 # get previous token
          if not tree.is_stopword(tk):
              wordPrev = tree.get_word(tk)
              lemmaPrev = tree.get_lemma(tk).lower()
              tagPrev = tree.get_tag(tk)
              feats.add("lprev=" + lemmaPrev)
              feats.add("wprev=" + wordPrev)
              feats.add("lpprev=" + lemmaPrev + "_" + tagPrev)
          else: feats.add("prev=stopword")
